...
// functional-spec.md 3.2節：スイーツ名からAIにcoffee_beansを1件選ばせ、理由とともに提案する。
// 1日10回まで（JST 0時境界）。上限チェックはAI呼び出し前、カウントはAI成功時の保存でのみ増える
@Service
public class PairingService {
    ...
    @Transactional
    public PairingSuggestionResponse suggest(Long userId, String sweetName) {
        Instant startOfToday = startOfTodayJst();
        long usedCount = pairingSuggestionRepository.countByUserIdAndCreatedAtGreaterThanEqual(userId, startOfToday);
        ...
        String aiResponseText = anthropicClient.complete(buildPrompt(sweetName, beans));
    ...
    private String buildPrompt(String sweetName, List<CoffeeBean> beans) {
